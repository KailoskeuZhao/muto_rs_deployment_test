...
class JoyTeleop(Node):
	...
	def user_jetson(self, joy_data):
		...
		if joy_data.buttons[11] == 1:
			if self.Buzzer_active == False:
				Buzzer_ctrl = Bool()
				Buzzer_ctrl.data =True
				self.pub_Buzzer.publish(Buzzer_ctrl)
				self.Buzzer_active=True
		else:
			if self.Buzzer_active == True:
		...
		# self.get_logger().info("xlinear_speed = {}, ylinear_speed = {}, angular_speed = {}".format(xlinear_speed,ylinear_speed,angular_speed))
		if self.Joy_active == True:
			self.get_logger().info("joy control now")
