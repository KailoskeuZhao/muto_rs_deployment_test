...
def build_visual_refinement_prompt(
    ...
    return (
        ...
        'or matches. Every requested visual attribute is mandatory. For an '
        'ordinary color description such as "green chair", require that color '
        'to be clearly attributable to the main object and dominant on its '
        'primary visible body or upholstery. Do not accept a match from small '
        'accents, frame hardware, armrests, legs, wheels, reflections, tinted '
        'lighting, or nearby/background objects unless the request explicitly '
        'names that part. Reject an occluded or ambiguous candidate rather than '
        'guessing. A semantic-kind match alone is insufficient when attributes '
        'were requested. Do not invent, rename, or normalize IDs. Return JSON only '
