...
The default detector is `yolo26m.pt`. Ultralytics candidate confidence filtering
...
Start the C++ registry as a separate node after the annotator:

```bash
ros2 launch sam2_object_registry object_registry_launch.py
```

It synchronizes the typed `/sam2/detections` message with
`/sam2/instance_pointcloud`, computes the mean XYZ position for every
