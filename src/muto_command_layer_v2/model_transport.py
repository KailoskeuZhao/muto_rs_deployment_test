...
def build_planner_prompt(board: MissionBoard, *, visual_available: bool) -> str:
    ...
    return (
        ...
        "search_progress counts POI-grid goals that actually succeeded, "
        "not a percentage and not proof of exhaustion. "
        "last_reason_code=poi_goal_succeeded means only that one POI goal "
        "made progress; it is not search exhaustion. Only "
        "last_reason_code=poi_exhausted (or an explicit "
        ...
        "approach_confirmed_object, go_to_point must carry the exact "
        "confirmed candidate_id. Its point may be null: the backend then "
        "resolves that candidate's current map position from the same "
        "registry revision and applies deterministic preflight policy; if a "
        "point is supplied it must match that position. Return "
